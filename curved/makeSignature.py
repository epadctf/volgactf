...
if __name__ == '__main__':
    ...
    z = int(hashlib.sha512("exit").hexdigest(), 16) >> 128
    zmark = int(hashlib.sha512("leave").hexdigest(), 16) >> 128

    (r, s) = import_cmd_signature("exit", ".")
    (rmark, smark) = import_cmd_signature("leave", ".")

    k = ((z - zmark) * invert((s - smark), curve.n)) % curve.n

    dA = int(((s * k - z % curve.n) * invert(r, curve.n)) % curve.n)
    print("private key: {}".format(hex(dA)))
